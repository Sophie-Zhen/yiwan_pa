...
## Current Focus
Dogfeed-day-1 fixes shipped 2026-05-21 (commit `ee5bd6d`): T-0 alerts (alerts='0' for "到点提醒"), `alerts` field editable via `update_inbox_item` (resets `alerted`), dispatch-layer dedup on `append_to_inbox` against Bug 1a recurrence. Still dogfeeding the Phase 1 push channels — observe before picking next post-v0.1 item.
