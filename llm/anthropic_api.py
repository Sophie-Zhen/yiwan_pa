...
TOOLS: list[dict[str, Any]] = [
    ...
    {
        "name": "add_contract",
        "description": "Add an annual contract to track for renewal (energy, broadband, home/car insurance). Use when Sophie wants to remember a contract's expiry and be reminded to shop around / switch — e.g. '记一下能源合同 7月2号到期', 'add my car insurance, renews 2027-06-15'. remind_on defaults to the expiry date; set it earlier for lead time to compare prices. current_price is free-form text (e.g. '0.42/kWh + €260 standing', '€540/year') so it can hold whatever the bill actually states.",
        "input_schema": {
            "type": "object",
            "properties": {
                "name": {"type": "string", "description": "Contract name incl. provider, e.g. 'Electric Ireland 电费', 'AXA car insurance'."},
                "contract_type": {"type": "string", "enum": ["energy", "broadband", "home_insurance", "car_insurance", "other"]},
                "expiry": {"type": "string", "description": "Expiry date YYYY-MM-DD."},
                "remind_on": {"type": "string", "description": "Date to remind, YYYY-MM-DD. Defaults to expiry; set earlier for lead time."},
                "current_price": {"type": "string", "description": "What she pays now, free-form (annual premium, unit rate, etc.)."},
                "notes": {"type": "string", "description": "Optional note."},
            },
    ...
    {
        "name": "list_contracts",
        "description": "List tracked contracts (default active), each with days_until_expiry. Use for '我有哪些合同', '什么快到期了', 'X 合同什么时候到期 / 现在多少钱'. DO NOT estimate from memory.",
        "input_schema": {
            "type": "object",
            "properties": {
                "status_filter": {"type": "string", "description": "Status to filter by; default 'active'. Pass null for all."},
            },
            "required": [],
        },
    },
    {
        "name": "renew_contract",
        "description": "Record that a contract was renewed: rotates the old price into prev_price (for year-over-year comparison), sets the new price and expiry, rolls the reminder forward, and re-arms next year's reminder. Use for '车险续约了，新到期 2027-06-15，今年 €560', 'I renewed energy, now 0.39/kWh'. If new_remind_on is omitted it keeps the same lead time relative to expiry as before.",
        "input_schema": {
            "type": "object",
            "properties": {
                "name": {"type": "string", "description": "Substring of the contract name."},
                "new_expiry": {"type": "string", "description": "New expiry date YYYY-MM-DD."},
                "new_current_price": {"type": "string", "description": "The new price, free-form."},
                "new_remind_on": {"type": "string", "description": "Optional new reminder date YYYY-MM-DD; defaults to preserving the old lead time."},
                "notes": {"type": "string", "description": "Optional updated note."},
            },
    ...
    },
    {
        ...
        "description": "Edit one field on a contract (type, expiry, remind_on, current_price, prev_price, status, notes). Use for corrections or to archive a contract ('don't track the broadband one anymore' → status=archived). For a renewal use renew_contract instead, which also rotates the price history.",
        "input_schema": {
            "type": "object",
            "properties": {
                "name": {"type": "string", "description": "Substring of the contract name."},
                "field": {"type": "string", "enum": ["type", "expiry", "remind_on", "current_price", "prev_price", "status", "notes"]},
                "value": {"type": "string", "description": "New value (empty to clear)."},
            },
    ...
    # === CWI logbook tools (Mountaineering Ireland Climbing Wall Instructor) ===
    # These write to a markdown store (data/cwi_log.md), NOT a Google Sheet.
    # Brief metadata only — the full DLOG reflective text is drafted in chat and
    # pasted by Sophie into MI's DLOG, never stored here.
    {
        "name": "log_instructed_session",
        "description": "Record a brief CWI logbook entry for an instructing/assisting session Sophie just delivered (top-rope taster, bouldering induction, group session, etc.), as she works toward the Mountaineering Ireland Climbing Wall Instructor (CWI) certificate. WORKFLOW: FIRST draft the DLOG reflective entry in chat (English, instructor-log voice) for her to paste into MI's DLOG — that draft is NOT stored. THEN call this to save only the brief metadata that powers the evening 'enter into DLOG' reminder and the CWI progress count. status starts 'pending' (not yet entered into the DLOG). One call per distinct session (two sessions in a day = two calls).",
        "input_schema": {
            "type": "object",
            "properties": {
                "date": {"type": "string", "description": "Session date YYYY-MM-DD (use today if she gives none)."},
                "venue": {"type": "string", "description": "Wall / centre name, e.g. 'Awesome Walls Dublin'. Used to count distinct walls toward the >=2 requirement."},
                "detail": {"type": "string", "description": "Short session kind, e.g. 'top-rope taster', 'bouldering induction'."},
                "role": {"type": "string", "enum": ["led", "assisted", "supervised"], "description": "Sophie's role in the session. Default 'assisted'."},
                "climbs_led": {"type": "integer", "description": "Climbs SHE personally led during the session, if any (usually 0 when instructing). Counts toward the 40-leads requirement."},
                "reflective": {"type": "boolean", "description": "Whether a reflective comment is written for this session on DLOG. Default true (each drafted entry is a reflective comment); counts toward the >=5-reflective requirement."},
                "large_public_facility": {"type": "boolean", "description": "True if the venue is a large public facility (e.g. Awesome Walls). At least one such venue is required among the instructing walls."},
                "notes": {"type": "string", "description": "Optional short note (participant count, anything to remember)."},
            },
    ...
    },
    {
        ...
        "description": "Record a brief CWI logbook entry for one of Sophie's PERSONAL climbing visits (her own training, NOT instructing) — counts toward the CWI personal-experience requirement (30 visits across >=3 walls, 40 climbs led). Triggered by '今天在 X 爬了，led 了 N 条' and similar. status starts 'pending' (not yet entered into MI DLOG).",
        "input_schema": {
            "type": "object",
            "properties": {
                "date": {"type": "string", "description": "Visit date YYYY-MM-DD (use today if none)."},
                "venue": {"type": "string", "description": "Wall / centre name. Counts toward distinct-walls (>=3)."},
                "climbs_led": {"type": "integer", "description": "Number of climbs she led on this visit (counts toward the 40-leads requirement). 0 if none / bouldering only."},
                "detail": {"type": "string", "description": "Optional short note on what she climbed, e.g. 'lead + bouldering'."},
    ...
    {
        "name": "cwi_progress",
        "description": "Report Sophie's progress toward the CWI assessment logbook requirements: instructed sessions (target 15 across >=2 walls incl. a large public facility, >=5 with reflective comments), personal climbing visits (target 30 across >=3 walls), and climbs led (target 40). Returns done / target / remaining for each. Use for '我的 CWI 进度怎么样 / 还差多少'. Targets are the official MI numbers — counted, not estimated.",
        "input_schema": {"type": "object", "properties": {}, "required": []},
    },
    {
        "name": "cwi_list_pending",
        "description": "List CWI logbook entries still in 'pending' status — logged here but not yet entered into Mountaineering Ireland's DLOG. Use for '哪些还没录' and to find the id(s) to pass to cwi_mark_recorded when she's entered some but not all.",
        "input_schema": {"type": "object", "properties": {}, "required": []},
    },
    {
        "name": "cwi_mark_recorded",
        "description": "Mark CWI logbook entries as entered into the MI DLOG, which stops the evening reminder for them. Use when Sophie says she's recorded them ('录好了', '都录进去了', 'logged it'). Omit ids to mark ALL pending entries (the common 'all done' case); pass specific ids (from cwi_list_pending) when she's entered only some.",
        "input_schema": {
            "type": "object",
            "properties": {
                "ids": {"type": "array", "items": {"type": "integer"}, "description": "Specific entry ids to mark recorded. Omit to mark all pending entries."},
            },
            "required": [],
        },
    },
...
def _execute_tool(name: str, args: dict[str, Any]) -> Any:
    ...
    if name == "add_contract":
        return add_contract(
            name=args["name"],
            contract_type=args["contract_type"],
            expiry=args["expiry"],
            remind_on=args.get("remind_on"),
            current_price=args.get("current_price"),
            notes=args.get("notes"),
        )
    if name == "list_contracts":
        return list_contracts(status_filter=args.get("status_filter", "active"))
    if name == "renew_contract":
        return renew_contract(
            name=args["name"],
            new_expiry=args["new_expiry"],
            new_current_price=args["new_current_price"],
            new_remind_on=args.get("new_remind_on"),
            notes=args.get("notes"),
        )
    ...
    if name == "log_instructed_session":
        return log_instructed_session(
            date=args["date"],
            venue=args["venue"],
            detail=args["detail"],
            role=args.get("role", "assisted"),
            climbs_led=args.get("climbs_led", 0),
            reflective=args.get("reflective", True),
            large_public_facility=args.get("large_public_facility", False),
            notes=args.get("notes"),
        )
    if name == "log_personal_climb":
        return log_personal_climb(
            date=args["date"],
            venue=args["venue"],
            climbs_led=args.get("climbs_led", 0),
    ...
    if name == "cwi_progress":
        return cwi_progress()
    if name == "cwi_list_pending":
        return cwi_pending_entries()
    if name == "cwi_mark_recorded":
        return cwi_mark_recorded(ids=args.get("ids"))
